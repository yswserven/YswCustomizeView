...
public class MyTextView extends View {
    private String mText = "365";
    private int mTextColor = Color.parseColor("#FF9C50");
    private int mBackgroundColor = Color.parseColor("#FFCCCB");
    private int mTextSize = UtilsTools.Sp2px(getContext(), 30);
    private int mMiddleMagin = UtilsTools.Dp2Px(getContext(), 14);
    private int mRectLineWidth = UtilsTools.Dp2Px(getContext(), 1);
    private int mRadius = UtilsTools.Dp2Px(getContext(), 5);
    ...
    private void DrawRectf(Canvas canvas) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            mPaint.setAntiAlias(true);
            mPaint.setStyle(Paint.Style.FILL);
            canvas.drawRoundRect(new RectF(i * (mSecondWidth + mMiddleMagin), 0, i * (mSecondWidth + mMiddleMagin) + mSecondWidth, mHeight),
                    mRadius, mRadius, mPaint);
            mPaint.reset();
            mPaint.setColor(mBackgroundColor);
            mPaint.setAntiAlias(true);
            mPaint.setStyle(Paint.Style.FILL);
            canvas.drawRoundRect(new RectF(i * (mSecondWidth + mMiddleMagin) + mRectLineWidth, 0 + mRectLineWidth,
                    i * (mSecondWidth + mMiddleMagin) + mSecondWidth - mRectLineWidth, mHeight - mRectLineWidth), mRadius, mRadius, mPaint);
            mPaint.reset();
            mPaint.setColor(mTextColor);
            mPaint.setAntiAlias(true);
