...
public class McoySnapPageLayout extends ViewGroup {
    // 监听手势的类 @author Ysw created at 2016/5/29 15:00
    private VelocityTracker mVelocityTracker;
    // 最大速度手指离开桌面滑动的距离 @author Ysw created at 2016/5/29 14:57
    private int mMaximumVelocity;
    // 应该是动画的时间1秒钟 @author Ysw created at 2016/5/29 14:59
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (yMoved) {
                    if (yDiff < 0 && mPageTop.isAtBottom() && mCurrentScreen == 0 || yDiff > 0 && mPageBottom.isAtTop() && mCurrentScreen == 1) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mTouchState != TOUCH_STATE_SCROLLING) {
                    ...
                    if (yDiff > gapBetweenTopAndBottom) {
                        mTouchState = TOUCH_STATE_SCROLLING;
                    }
                }
                // 手指拖动屏幕的处理
                if ((mTouchState == TOUCH_STATE_SCROLLING)) {
                    ...
                    } else if (mPageBottom != null && mPageBottom.isAtTop()) {
                        scrollBy(0, deltaY);
                    }
                }
                break;
...
//        }
        ...
        mScroller.startScroll(0, cy, 0, delta, Math.abs(delta) / 3);
        invalidate();
    }
